# LLM ThreatIntel Validation Report

- Run time UTC: `2026-08-19T03:48:14+00:00`
- Commit SHA: `fcbfc10ee6607a15e143eb4c516467b5826b9bf5`
- Mode: `full`
- Validation version: `1.0.0`
- Overall result: `pass`
- Hard failures: `0`
- Warnings: `5`
- Review required: `4`
- Reports checked: `4`
- Reports skipped: `222`
- Reports newly validated: `3`
- IOC duplicates found: `0`
- Source URLs checked: `17`

No files were removed or destructively modified.

## Findings

- **WARN** `post-excerpt-long` `data/posts-index.json` `2026-08-19-autonomous-ai-agents-cyberattack-operators`: post excerpt is long (1300 characters)
- **WARN** `post-excerpt-long` `data/posts-index.json` `2026-08-19-hugging-face-autonomous-agent-breach-openai-gpt`: post excerpt is long (911 characters)
- **WARN** `post-tlp-public-warning` `data/posts-index.json` `2026-08-15-taiwan-autonomous-ai-agent-nuclear-attack-hermes-openclaw`: public feed post uses non-clear TLP: TLP:AMBER
- **WARN** `post-tlp-public-warning` `data/posts-index.json` `2026-07-26-nation-state-llm-operationalization-gtig-2026`: public feed post uses non-clear TLP: TLP:AMBER
- **WARN** `post-excerpt-long` `data/posts-index.json` `2026-06-30-cordyceps-cicd-github-actions-supply-chain-vulnerability-june-2026`: post excerpt is long (1205 characters)
- **REVIEW** `evidence-url-review` `posts/2026-08-19-autonomous-ai-agents-cyberattack-operators.md` `2026-08-19-autonomous-ai-agents-cyberattack-operators`: 1 source URL(s) require review
- **REVIEW** `evidence-url-review` `posts/2026-08-19-ghostapproval-symlink-ai-coding-agents.md` `2026-08-19-ghostapproval-symlink-ai-coding-agents`: 1 source URL(s) require review
- **REVIEW** `evidence-url-review` `posts/2026-08-19-hugging-face-autonomous-agent-breach-openai-gpt.md` `2026-08-19-hugging-face-autonomous-agent-breach-openai-gpt`: 1 source URL(s) require review
- **REVIEW** `evidence-source-review-required` `posts/2026-08-05-stardust-chollima-mastra-npm-supply-chain-dprk-august-2026.md` `2026-08-05-stardust-chollima-mastra-npm-supply-chain-dprk-august-2026`: no source URL was accessible; manual review or alternate evidence is required

## Human Review Queue

```text
Report: 2026-08-19-autonomous-ai-agents-cyberattack-operators
Problem: evidence-url-review
Validator finding: 1 source URL(s) require review
Recommended options:
[ ] Keep report and add/confirm valid source
[ ] Add manual evidence override
[ ] Rewrite report with supported claims only
[ ] Mark report as unverified
[ ] Remove report from feed after Lucas approval
```

```text
Report: 2026-08-19-ghostapproval-symlink-ai-coding-agents
Problem: evidence-url-review
Validator finding: 1 source URL(s) require review
Recommended options:
[ ] Keep report and add/confirm valid source
[ ] Add manual evidence override
[ ] Rewrite report with supported claims only
[ ] Mark report as unverified
[ ] Remove report from feed after Lucas approval
```

```text
Report: 2026-08-19-hugging-face-autonomous-agent-breach-openai-gpt
Problem: evidence-url-review
Validator finding: 1 source URL(s) require review
Recommended options:
[ ] Keep report and add/confirm valid source
[ ] Add manual evidence override
[ ] Rewrite report with supported claims only
[ ] Mark report as unverified
[ ] Remove report from feed after Lucas approval
```

```text
Report: 2026-08-05-stardust-chollima-mastra-npm-supply-chain-dprk-august-2026
Problem: evidence-source-review-required
Validator finding: no source URL was accessible; manual review or alternate evidence is required
Recommended options:
[ ] Keep report and add/confirm valid source
[ ] Add manual evidence override
[ ] Rewrite report with supported claims only
[ ] Mark report as unverified
[ ] Remove report from feed after Lucas approval
```
