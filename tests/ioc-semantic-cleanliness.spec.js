...
test.describe("IOC semantic cleanliness", () => {
  ...
  test("false-positive platforms absent from package IOC feed", async ({
    page,
  }) => {
    ...
    const values = await page
      .locator(".ioc-table tbody tr .ioc-value")
      .allTextContents();
  ...
  test("false-positive platforms absent from package exports", async ({
    page,
  }) => {
    await page.selectOption("#ioc-type-filter", "package");
  ...
  test("bare OpenRouter hosts absent from domain IOC feed", async ({
    page,
  }) => {
    await page.goto("/#ioc-feed?type=domain&status=all");
    await page.waitForSelector(".ioc-table");
  ...
  test("specific malicious Hugging Face repository remains in URL feed", async ({
    page,
  }) => {
    await page.selectOption("#ioc-type-filter", "url_path");
    await page.selectOption("#ioc-status-filter", "all");
  ...
  test("ollama and grok remain searchable in report content", async ({
    page,
  }) => {
    await page.goto("/");
    await page.locator("#header-search-input").fill("ollama");
    await page.waitForTimeout(400);
    const ollamaText = await page
      .locator("#search-results, .search-results, main")
      .textContent();
    expect((ollamaText || "").toLowerCase()).toContain("ollama");

    await page.locator("#header-search-input").fill("grok");
    await page.waitForTimeout(400);
    const grokText = await page
      .locator("#search-results, .search-results, main")
      .textContent();
